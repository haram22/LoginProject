...
//        tf.addTarget(self, action: #selector(textFieldEditingChanged(_:)), for: .editingChanged)
    ...
    private lazy var passwordTextField: UITextField = {
        ...
        tf.autocorrectionType = .no
        tf.spellCheckingType = .no
        tf.isSecureTextEntry = true // obscure
    ...
    private lazy var loginButton: UIButton = {
        ...
        // radius는 주로 이렇게 둘이 같이 쓴다.
        button.layer.cornerRadius = 5
        button.clipsToBounds = true // 어떤 영역 내로 잘라낸다는 뜻.
        //
        button.layer.borderWidth = 1
        button.layer.borderColor = #colorLiteral(red: 0.2, green: 0.2, blue: 0.2, alpha: 1) // 로그인 버튼의 border
        button.setTitle("로그인", for: .normal)
        button.titleLabel?.font = UIFont.boldSystemFont(ofSize: 16)
        // 처음에는 버튼이 동작하지 않게 설정한다.
...
//            passwordInfoLabel.centerYAnchor.constraint(equalTo: passwordTextFieldView.centerYAnchor),
    ...
    @objc func resetButtonTapped(){
        ...
        // alert에 올리는 것. addSubview의 역할과 비슷하다.
...
extension ViewController: UITextFieldDelegate {
    // MARK: - 유저가 입력을 시작하는 경우
    func textFieldDidBeginEditing(_ textField: UITextField) {
        // 유저가 선택힌 필드가 이메일 텍스트 필드라면
